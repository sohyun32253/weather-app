  ...
  return (
    <main className="min-h-screen" style={{background: theme.background,}}>
      <div className="mx-auto w-full max-w-md px-4 py-6 md:max-w-3xl lg:max-w-6xl lg:px-8 lg:py-10">
        <Link
          href="/"
          className="mb-6 inline-flex items-center text-sm text-accent hover:underline md:text-base text-white"
        >
          ← 목록으로
        </Link>
        ...
        <div className="lg:grid lg:grid-cols-5 lg:items-start lg:gap-10 xl:gap-14">
          <section className="mb-8 lg:col-span-2 lg:mb-0 lg:sticky lg:top-8">
            <div className="rounded-2xl border border-border bg-card p-5 shadow-md md:p-8" style={{background: theme.background}}>
              ...
              <p className="mt-2 text-sm text-muted md:text-base text-white/80">
                {formatDateTime(current.time)} 기준
              </p>

              <div className="mt-6 md:mt-8">
                <p className="text-2xl leading-none md:text-2xl lg:text-3xl text-white/90">
                  {getWeatherLabel(current.weather_code)}
                </p>
                <p className="mt-4 text-4xl font-bold tabular-nums md:text-5xl lg:text-6xl text-white">
              ...
              <div className="mt-6 flex flex-wrap gap-4 border-t border-border pt-5 text-sm md:gap-6 md:text-base lg:mt-8 lg:gap-8">
                ...
                <div>
                  <p className="text-muted text-white/90">습도</p>
                  <p className="font-semibold tabular-nums text-white/90">
          ...
          {weeklyForecast.length > 0 && (
            <section className="lg:col-span-3">
              <div className="rounded-2xl border border-border bg-card p-5 shadow-md md:p-6 lg:p-8"  style={{background: theme.background}}>
                <WeeklyForecast days={weeklyForecast} theme={theme}/>
              </div>
            </section>
          )}
        </div>
